...
export default function ViewCategory() {
  // let [orderModal, setOrderModal] = useState(false);
    ...
    let getColor = () => {
    axios
      .get(`${baseURL}color/view`)
      .then((res) => res.data)
      .then((finalRes) => {
        console.log(finalRes);

        if (finalRes._status) {
          setData(finalRes.colorData);
  ...
  let getCheckValue=(e)=>{
    if(e.target.checked){
        ...
        setAllIds(allIds.filter((v)=>v!=e.target.value))//Array
    }
  ...
  let deleteRow=()=>{
    if(allIds.length>=1){
        axios.post(`${baseURL}color/multi-delete`,{ids:allIds})
        .then((res)=>res.data)
        .then((finalRes)=>{
           if(finalRes._status){
      ...
      alert("Please Select al least one check box")
    }
  }
  ...
  let statusChange=()=>{
    if(allIds.length>=1){
        axios.post(`${baseURL}color/status-change`,{ids:allIds})
        .then((res)=>res.data)
        .then((finalRes)=>{
           if(finalRes._status){
        ...
        <div className="max-w-[1220px] mx-auto py-5">
          <div className='flex item-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400'>
            ...
            <div className='flex justify-between '>
              ...
              <button onClick={statusChange} type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"> Change Status</button>
              <button onClick={deleteRow} type="button" className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete </button>
            </div>
          </div>
